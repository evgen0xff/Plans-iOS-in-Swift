...
struct KeychainItem {
    ...
    init(_ service: String? = nil, key: String, accessGroup: String? = nil) {
    ...
    static func getStringWith(key: String) -> String? {
        do {
            return try KeychainItem(key: key).readItem()
        } catch {
            return nil
        }
    }
    
    static func deleteStringWith(key: String) {
        do {
            try KeychainItem(key: key).deleteItem()
        } catch {
            print("Unable to delete key-value from keychain")
        }
    }
